...
public class Game {
	...
	public static void main(String[] args) {
		Card[] field = new Card[52]; // creating a deck
		fillDeck(field); // filling the deck with cards to deal to the players;
		shuffleDeck(field); // shuffling the deck;
		Player p1 = new Player("Strahil"); //creating players
		Player p2 = new Player("Pesho");
		Player winner = null;
		
		for (int i = 0; i < field.length; i++) {  //dealing cards to the players
			if(i%2 == 0) p2.addToHand(field[i]);
			else p1.addToHand(field[i]);
		}
		field = new Card[52]; // creating a field for the cards to be played in
		System.out.println("===" + p1.getName() + " VS " + p2.getName() + "===");
		do{
			// Visualizing the hands of the players
			p1.printHand(); 
			p2.printHand();
			// Showing the current match-up
			System.out.println(p1.getNextCard().getSuit() + " VS " + p2.getNextCard().getSuit());
			//Adding the cards to the field
			addToPot(field, p1.getNextCard());
			addToPot(field, p2.getNextCard());
			//Comparing the cards to decide round winner
		...
		System.out.println( p1.getName() + " cards: " + p1.getWon().size());
		System.out.println( p2.getName() + " cards: " + p2.getWon().size());
	}
